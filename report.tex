...
\begin{enumerate}[leftmargin=2em]
    ...
    \item Upload \texttt{server.py}, \texttt{db.py}, \texttt{.env}, \texttt{cert.pem}, \texttt{key.pem}, and client files
    ...
    \item Start the server on port 5000 (HTTPS): \texttt{PORT=5000 python3 server.py}
    \item Verify health: \texttt{curl -k https://localhost:5000/health}
\end{enumerate}
...
./load_balancer \
  -backends https://172.17.0.39:5000,
...
{"backends":[{"url":"https://172.17.0.39:5000","alive":true}]}
\end{lstlisting}

\textbf{LB Metrics after experiment:}
\begin{lstlisting}[style=jsonstyle]
{"backend_errors":35, "failed":740, "latency_count":4295, "success":4260, "total":5000}
\end{lstlisting}
...
./load_balancer -backends https://172.17.0.39:5000,\
    https://172.17.0.40:5000,https://172.17.0.41:5000 -port 8082
...
\begin{lstlisting}[style=jsonstyle]
{"backends":[
  {"url":"https://172.17.0.39:5000","alive":true},
  {"url":"https://172.17.0.40:5000","alive":true},
  {"url":"https://172.17.0.41:5000","alive":true}
]}
\end{lstlisting}

\textbf{LB Metrics after experiment:}
\begin{lstlisting}[style=jsonstyle]
{"backend_errors":0, "failed":0, "latency_count":5000, "success":5000, "total":5000}
\end{lstlisting}
...
1_backend,4232,768,252.3,15.4%,514.0ms,1404.0ms,2599.0ms
3_backends,4967,33,371.8,0.7%,398.0ms,1198.0ms,2695.0ms
\end{lstlisting}
...
\midrule
Successful Requests & 4,232        & 4,967        & +17.4\% \\
Failed Requests     & 768          & 33           & $-$95.7\% \\
Throughput (RPS)    & 252.3        & 371.8        & +47.4\% \\
Dropout (\%)        & 15.4\%       & 0.7\%        & $-$14.7 pp \\
p50 Latency         & 514 ms       & 398 ms       & $-$22.6\% \\
p95 Latency         & 1,404 ms     & 1,198 ms     & $-$14.7\% \\
p99 Latency         & 2,599 ms     & 2,695 ms     & +3.7\% \\
Backend Errors      & 35           & 0            & $-$100\% \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Throughput improved by 47\%.}
With three backends, the load balancer distributes requests across three servers. Each server handles approximately one-third of the traffic, allowing more requests to be processed in parallel. Throughput increased from 252.3 to 371.8~RPS.

\item \textbf{Failures reduced by 96\%.}
The single backend received all 5,000 requests alone. Under heavy concurrency (200 workers), the backend became a bottleneck, causing 768 timeouts. With three backends, failures dropped to just 33 --- a 95.7\% reduction.

\item \textbf{Tail latency (p95) improved by 15\%.}
The p95 latency dropped from 1,404\,ms to 1,198\,ms ($-$14.7\%). This indicates that the worst-case request times improve substantially with load distribution. The p99 remained comparable (2,599\,ms vs.\ 2,695\,ms) due to occasional TLS handshake overhead.

\item \textbf{Median latency (p50) improved by 23\%.}
The p50 latency dropped from 514\,ms to 398\,ms, a notable improvement. With load distributed across three backends, each server has lower queue contention, resulting in faster median response times.

\item \textbf{Backend errors dropped from 35 to 0.}
With a single backend, high concurrency caused 35 connection and timeout errors. Distributing the load completely eliminated backend-level errors.
...
\begin{itemize}[leftmargin=2em]
    \item The back-end (\texttt{server.py}) runs on Sys2, Sys3, and Sys4 on port 5000 (HTTPS).
    \item The load balancer on Sys1 (port 8082) accepts HTTP and proxies to HTTPS backends.
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item Results confirm that load balancing provides \textbf{47\% higher throughput}, \textbf{96\% fewer failures}, and \textbf{15\% lower tail latency} under heavy load.
\end{enumerate}
