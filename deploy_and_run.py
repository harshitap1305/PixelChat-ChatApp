...
def setup_backend(port_ssh):
    ...
    ssh_exec(port_ssh, "pip3 install --break-system-packages fastapi uvicorn websockets")
    ssh_exec(port_ssh, "pkill -f server.py || true")
    
    # Run the server on port 8081
    ssh_exec(port_ssh, "export PORT=8081; nohup python3 group-chat-app/server/server.py > group-chat-app/server.log 2>&1 &")
    time.sleep(2)
    ssh_exec(port_ssh, "curl -s http://localhost:8081/health || echo 'Server failed to start'")
...
def setup_load_balancer(port_ssh):
    ...
    backends = "http://sys2:8081,http://sys3:8081,http://sys4:8081"
    start_cmd = f"nohup ./group-chat-app/load_balancer -backends {backends} -port 8080 > group-chat-app/lb.log 2>&1 &"
    ssh_exec(port_ssh, start_cmd)
    time.sleep(2)
    ssh_exec(port_ssh, "curl -s http://localhost:8080/lb/health || echo 'LB failed to start'")
